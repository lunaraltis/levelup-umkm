...
export default function Hero({ initialData }: { initialData: HeroContent }) {
  const [currentSlide, setCurrentSlide] = useState(0);
  const brands = initialData.brands.filter(Boolean);
      ...
      <div className="container">
        ...
        <AnimateOnScroll delay={0.7}>
          <div style={{ display: "flex", gap: "1rem", justifyContent: "center", flexWrap: "wrap" }}>
            <Link
              href="#pricing"
              onClick={() => trackEvent("cta_click", { source: "hero", target: "pricing" })}
              className="btn btn-primary"
              style={{ padding: "1rem 2rem" }}
            >
              {initialData.primaryCtaLabel} <ArrowRight size={18} />
            </Link>
            <Link
              href="#about"
              onClick={() => trackEvent("cta_click", { source: "hero", target: "about" })}
              className="btn btn-outline"
              style={{ padding: "1rem 2rem" }}
            >
              {initialData.secondaryCtaLabel}
            </Link>
          </div>
        </AnimateOnScroll>
